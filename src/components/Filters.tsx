...
export const Filters: React.FC<FiltersProps> = ({ onFilter }) => {
  ...
  return (
    <div className="w-full mx-auto px-2 m-4 sm:p-6 lg:p-4 space-y-4 bg-white shadow rounded-lg flex justify-between items-center flex-wrap">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <Select
            isMulti
            options={
              objectTypesOptions as { value: ObjectType; label: ObjectType }[]
            }
            value={selectedObjectTypes.map((type) => ({
              value: type as ObjectType,
              label: type as ObjectType,
            }))}
            onChange={(options) =>
              setSelectedObjectTypes(
                options?.map((o) => o.value as ObjectType) || []
              )
            }
        ...
        <div>
          ...
          <Select
            ...
            value={selectedOrbitCodes.map((code) => ({
              value: code,
              label: code,
            }))}
            onChange={(options) =>
              setSelectedOrbitCodes(options?.map((o) => o.value) || [])
            }
